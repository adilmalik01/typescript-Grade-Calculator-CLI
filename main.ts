...
if (prompt.mathMarks < 0 || prompt.mathMarks > 100 ||
    ...
    let obtainMarks: number = prompt.mathMarks + prompt.englishMarks + prompt.UrduMarks + prompt.SSTMarks + prompt.IslamiatMarks;
    ...
    if (percentage >= 90 && percentage <= 100) {
        ...
        console.log(
            chalk.green.bold.italic(`percentage :${percentage.toFixed(2)}% \nGrade : A+`)
        );
        ...
        console.log(
            chalk.red.bold.italic(`percentage :${percentage.toFixed(2)}%  \nGrade :  D`)
        );

    }
    else {
        console.log(
            chalk.bgRed(`percentage :${percentage.toFixed(2)}%  YOU ARE FAIL`)
        );
    }
